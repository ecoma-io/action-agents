...
| Review run artifact — architecture section | persist: the decision basis (the verdict, the stale flag, the report digest over the raw bytes, the baseline digest, the tool version, both provenance commits, both policy fingerprints, the bucket counts, `policyChanged`); fingerprint: the detail lists (identity and count persist; the full site lists drop to the digest-referenced workflow artifact); never: raw model text (there is none)                                            | the section is digests and counts over a deterministic tool's output — the basis outlives the runner, the bulk rides the workflow artifact the report digest names, and no model text exists to keep (#529) |
