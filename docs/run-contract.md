...
| `harmonise` | config-absent, or any throw the run did not declare                           | a `failed` record, then the red error — the boundary writer pins F-15         |
...
| F-02  | config-invalid/absent     | `refused` (triage, review); harmonise: `failed` via the boundary record            | no config, no run                                                                                                                                                                                                                                            |
...
- **One record per run, at every terminal point the action declares.** A
  ...
  a mid-run defect — reaches the boundary writer, which records `failed`
  and lets the original error fail the step, so the record never masks the
  throw it records. Only a run that dies before it holds the facts a record
  is built from — the entrypoint's input and context reads — and a red run
  whose record write itself fails stay unrecorded; the upload's
  `if-no-files-found: ignore` keeps those green. Review has no declared
  failure record yet; its failure-record path is its own change.
...
| harmonise's record | `harmonise/src/run-record.mjs` | 3                           | `harmonise-record-*.json` | every terminal point: publication, partial exit, all-in-step skip, dry run — and the red terminals, where the boundary writer records `failed` |
