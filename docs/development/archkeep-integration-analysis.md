...
| `drift` / `reconcile`                                    | stable                             | none                        | remediation context (ranked law-edit candidates, never auto-applied)                                                                                                                                 | —                      | doc-vs-intent mismatch evidence (P4: explored, deferred — §6.4) | OPT                                      |
...
- **Harmonise** is unchanged through P1–P3 (it reads through its own
  ...
  pull request the human merges. **P4's outcome** (#551, 2026-09-13):
  explored and deferred with reason, recorded on
  [harmonise's development page](harmonise.md) — the judgment axis
  (per-language equivalence to the source, never source truth), the
  evidence channel (a first working-tree read for an action with none),
  the surface (the mismatch is about the source, which the action never
  edits) and the consumer ([ADR 002](../adr/002-no-intelligence-layer.md)'s
  evidenced-consumer standard, unmet) each fail the case on their own.
  The revisit trigger is a dogfooded repository with translated
  architecture documents in its map suffering a real staleness event;
  harmonise stays at zero archkeep invocations (§6.6).
