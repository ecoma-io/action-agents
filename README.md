...
a diff properly, answering someone in the language they wrote in, keeping the
translated docs from drifting apart. A model can do most of it — but handing a
model a write token is only safe if what it may do is bounded by something other
than the prompt. These four actions draw that boundary in code: **model output
becomes comment text and never selects an API call**, and everything read from a
thread or a diff is evidence, never instruction.
...
`.github/action-agents.json`, and every action runs without it. Prose settings —
a review rubric, the language a document is harmonised against — are markdown
files an input points at,
...
|                                          |                                                                                                                                      |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| [**`triage`**](triage/action.yaml)       | Classifies issues and pull requests, applies semantic labels, assesses size, and manages lifecycle according to your configuration.  |
...
|                                       |                                                                       |
| ------------------------------------- | --------------------------------------------------------------------- |
| [**Security**](SECURITY.md)           | The threat model, the two ceilings, and how to report a vulnerability |
